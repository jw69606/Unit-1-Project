...
public class ProjectUnit1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter in the initial temperature of the water in Celsius.");
		double initialTemp = reader.nextDouble();
		if(initialTemp < -273.14)
			initialTemp = -273.14;
		
		System.out.println("Enter in the final temperature of the water in Celsius.");
		...
		System.out.println("Initial Temperature: " + initialTemp + "C. The initial phase is " + initialPhase + ".");
		System.out.println("Final Temperature: " + finalTemp + "C. The final phase is " + endPhase + "." + "\n");
		...
		if (initialPhase .equals("solid")) {
			heatEnergy += tempChangeSolid(mass, initialTemp, finalTemp, endPhase, endothermic);
		
			if (!endPhase.equals("solid")) {
				heatEnergy += fusion(mass, endothermic);
				heatEnergy += tempChangeLiquid(mass, 0, finalTemp, endPhase, endothermic);
			}
		
			if(endPhase.equals("vapor")) {
				heatEnergy += vaporization(mass, endothermic);
				heatEnergy += tempChangeVapor(mass, 100, finalTemp, endPhase, endothermic);
			}
		}//end of section for initial phase: solid
		
		//inital phase: liquid
		if (initialPhase .equals("liquid")) {
			heatEnergy += tempChangeLiquid(mass, initialTemp, finalTemp, endPhase, endothermic);
			
			if(endPhase.equals("solid")) {
				heatEnergy += fusion(mass, endothermic);
				heatEnergy += tempChangeSolid(mass, 0, finalTemp, endPhase, endothermic);
			}
			if(endPhase.equals("vapor")) {
				heatEnergy += vaporization(mass, endothermic);
				heatEnergy += tempChangeVapor(mass, 100, finalTemp, endPhase, endothermic);
		...
		if (initialPhase .equals("vapor")) {
			heatEnergy += tempChangeVapor(mass, initialTemp, finalTemp, endPhase, endothermic);
		
			if (!endPhase.equals("vapor")) {
				heatEnergy += vaporization(mass, endothermic);
				heatEnergy += tempChangeLiquid(mass, 100, finalTemp, endPhase, endothermic);
			}
	...
	public static double fusion(double mass,boolean endothermic) {
		double energyChange;
		if (endothermic) { // an if statement with one command does not need {} while multiple do
